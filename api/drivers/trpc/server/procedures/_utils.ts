...
import { createServerContext } from 'api/context/server/mod.ts'
import { createWebhookContext } from 'api/context/webhook/mod.ts'
import { createWorkerContext } from 'api/context/worker/mod.ts'
...
export async function withServerContext({ ctx, signal, next }: {
  ...
  try {
    ...
    throw new Error(error instanceof Error ? error.message : 'An error occurred while creating server context')
  }
}
