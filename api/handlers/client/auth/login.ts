...
export default async function* login(
  { ctx, input }: ClientRequest<Input, ClientAuthContext & LoggerContext>,
): ClientResponse {
  ...
  yield ctx.log.loader('🔑 Starting OAuth flow...')
  ...
  yield ctx.log.loader(`📱 Please complete authentication in your browser...\n\tOpen this URL: ${data.url}`)
  ...
  while (attempts < maxAttempts) {
    ...
    if (newSession?.access_token) {
      yield `🔑 Authenticated as ${newSession.user?.email || 'user'}.`
      return
    }
