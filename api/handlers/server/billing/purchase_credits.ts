...
import { BillingClient, PaymentMetadataV1 } from 'core/application/billing/utils.ts'
import { BillingClientRepository } from 'core/repositories/mod.ts'
import type { BillingClient as BillingClientDTO } from 'core/entities/billing/client.ts'
...
export default async function* PurchaseCredits({ ctx, input, signal }: ServerRequest<Input>): ServerResponse<Output> {
  ...
  const billingClientRepo = new BillingClientRepository(
    ctx.kube.client,
  ...
  await billingClientRepo.upsert(input.client as BillingClientDTO, signal)
