...
import z from 'zod'
import { PaymentMetadataV1 } from 'core/application/billing/utils.ts'
...
export default async function* ({ ctx, input }: WebhookRequest<Input>): WebhookResponse<Output> {
  ...
  if (payment.status === 'paid') {
    ...
    const projectRepo = new ProjectRepository(ctx.kube.client, ownerId)
    ...
    const newBalance = await projectRepo.addCredits(
      projectId,
