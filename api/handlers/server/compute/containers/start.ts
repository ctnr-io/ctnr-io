...
import { ensureHorizontalPodAutoscaler } from 'core/adapters/kubernetes/kube-client.ts'
import { checkUsage } from 'core/application/billing/usage.ts'
import { extractDeploymentResourceUsage } from 'core/application/billing/resource.ts'
...
export default async function* startContainer(request: ServerRequest<Input>): ServerResponse<void> {
  ...
  // Scale deployment
  await repo.scale(name, minReplicas)
...
async function waitForDeployment({ repo, name, predicate, signal }: {
  repo: ContainerRepository
  name: string
  predicate: (deployment: Deployment) => boolean | Promise<boolean>
  signal: AbortSignal
}): Promise<Deployment> {
  const deploymentWatcher = await repo.watchDeployments({
