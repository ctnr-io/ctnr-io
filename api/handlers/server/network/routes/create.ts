...
import { isDomainVerified, verifyDomainOwnership } from 'core/adapters/dns/verification.ts'
...
import { RouteRepository } from 'core/repositories/route_repository.ts'
...
export default async function* createRoute(request: ServerRequest<Input>): ServerResponse<void> {
  ...
  const routeRepo = new RouteRepository(ctx.kube.client, ctx.project)
  ...
  try {
    ...
    // Create route using repository (handles ctnr.io hostnames)
